...
import { DealerCreditBadge } from "@/components/dealers/dealer-credit-badge";
import { DealerEmptyState } from "@/components/dealers/dealer-empty-state";
import { DealerSearch } from "@/components/dealers/dealer-search";
...
function isSortField(value: string): value is DealerSortField {
  return (DEALER_SORT_FIELDS as readonly string[]).includes(value);
}

const columnHelper = createColumnHelper<DealerDTO>();
...
export function DealerTable() {
  ...
  const [sortBy, setSortBy] = useState<DealerSortField>("createdAt");
  const [sortOrder, setSortOrder] = useState<SortOrder>("desc");

  const [result, setResult] = useState<PaginatedResult<DealerDTO> | null>(null);
  ...
  useEffect(() => {
    ...
    setStatus((current) => (current === "success" ? "loading" : "loading"));
  ...
  const columns = useMemo(
    () => [
      columnHelper.accessor("dealerCode", {
        id: "dealerCode",
        header: () => t("dealers.column.dealerCode"),
        enableSorting: true,
        cell: (info) => (
      ...
      columnHelper.accessor("companyName", {
        id: "companyName",
        header: () => t("dealers.column.companyName"),
        enableSorting: true,
        cell: (info) => {
      ...
      columnHelper.accessor("mobile", {
        id: "mobile",
        header: () => t("dealers.column.mobile"),
        enableSorting: false,
        cell: (info) => (
      ...
      columnHelper.accessor("territory", {
        id: "territory",
        header: () => t("dealers.column.territory"),
        enableSorting: false,
        cell: (info) => {
          const dealer = info.row.original;
          return (
      ...
      columnHelper.accessor((row) => row.credit.creditLimit, {
        id: "creditLimit",
        header: () => t("dealers.column.creditLimit"),
        enableSorting: true,
        cell: (info) => (
          <span className="tabular-nums text-slate-700 dark:text-slate-300">
      ...
      columnHelper.accessor((row) => row.credit.currentBalance, {
        id: "currentBalance",
        header: () => t("dealers.column.currentBalance"),
        enableSorting: true,
        cell: (info) => (
      ...
      columnHelper.display({
        id: "creditStatus",
        header: () => t("dealers.column.creditStatus"),
        enableSorting: false,
        cell: (info) => {
  ...
  const numericColumns = useMemo(
    () => new Set(["creditLimit", "currentBalance"]),
    [],
  );
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <DealerSearch value={search} onChange={handleSearchChange} />
        {status === "success" && total > 0 && (
          <p className="text-xs text-slate-500 dark:text-slate-400">
            {t("dealers.pagination.showing")}{" "}
            <span className="font-medium text-slate-700 tabular-nums dark:text-slate-300">
              {rangeStart}–{rangeEnd}
            </span>{" "}
            {t("dealers.pagination.of")}{" "}
            <span className="font-medium text-slate-700 tabular-nums dark:text-slate-300">
              {total}
            </span>{" "}
            {t("dealers.pagination.dealers")}
          </p>
        )}
      </div>
      ...
      {status === "error" && (
        <div className="flex flex-col items-center justify-center rounded-xl border border-rose-200/80 bg-rose-50/50 px-6 py-16 text-center dark:border-rose-900/60 dark:bg-rose-950/30">
          ...
          <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">
            {t("dealers.error.title")}
          </h3>
          <p className="mt-1 max-w-sm text-sm text-slate-500 dark:text-slate-400">
            {t("dealers.error.description")}
          </p>
          <button
            type="button"
            onClick={() => {
          ...
          >
            {t("dealers.error.retry")}
          </button>
        </div>
      )}

      {status !== "error" && !isInitialLoading && total === 0 && (
        <DealerEmptyState searchActive={searchActive} />
      )}
