"use client";

import { AlertTriangle, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { useParams } from "next/navigation";
import { useEffect, useState } from "react";

import { DealerForm } from "@/components/dealers/dealer-form";
import { PageContainer } from "@/components/layout/page-container";
import { useLanguage } from "@/contexts/LanguageContext";
import { getDealer } from "@/lib/actions/dealers/get-dealer";
import type { DealerDTO } from "@/types/dealer";

type LoadState =
  | { status: "loading" }
  | { status: "error" }
  | { status: "ready"; dealer: DealerDTO };

function FormSkeleton() {
  return (
    <div className="mx-auto w-full max-w-3xl space-y-6" aria-hidden="true">
      {[0, 1, 2, 3].map((section) => (
        <div
          key={section}
          className="overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900"
        >
          <div className="border-b border-slate-100 px-5 py-4 dark:border-slate-800/80">
            <div className="h-4 w-40 animate-pulse rounded bg-slate-200 dark:bg-slate-800" />
            <div className="mt-2 h-3 w-56 animate-pulse rounded bg-slate-100 dark:bg-slate-800/70" />
          </div>
          <div className="grid grid-cols-1 gap-x-6 gap-y-5 p-5 sm:grid-cols-2">
            {[0, 1].map((field) => (
              <div key={field} className="space-y-2">
                <div className="h-3.5 w-24 animate-pulse rounded bg-slate-100 dark:bg-slate-800/70" />
                <div className="h-9 w-full animate-pulse rounded-lg bg-slate-200 dark:bg-slate-800" />
              </div>
            ))}
          </div>
        </div>
      ))}
    </div>
  );
}

export default function EditDealerPage() {
  const { t } = useLanguage();
  const params = useParams<{ dealerCode: string }>();
  const dealerCode = Array.isArray(params.dealerCode)
    ? params.dealerCode[0]
    : params.dealerCode;

  const [state, setState] = useState<LoadState>({ status: "loading" });

  useEffect(() => {
    let cancelled = false;

    void (async () => {
      if (!dealerCode) {
        if (!cancelled) {
          setState({ status: "error" });
        }
        return;
      }

      setState({ status: "loading" });

      const result = await getDealer({ dealerCode });
      if (cancelled) {
        return;
      }
      if (result.success) {
        setState({ status: "ready", dealer: result.data });
      } else {
        setState({ status: "error" });
      }
    })();

    return () => {
      cancelled = true;
    };
  }, [dealerCode]);

  return (
    <PageContainer
      title={t("dealers.form.editTitle")}
      description={t("dealers.form.editSubtitle")}
      actions={
        <Link
          href="/dealers"
          className="inline-flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-sm font-medium text-slate-700 shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
        >
          <ArrowLeft aria-hidden="true" className="size-4" />
          {t("dealers.form.backToList")}
        </Link>
      }
    >
      {state.status === "loading" && <FormSkeleton />}

      {state.status === "error" && (
        <div className="mx-auto flex w-full max-w-3xl flex-col items-center justify-center rounded-xl border border-rose-200/80 bg-rose-50/50 px-6 py-16 text-center dark:border-rose-900/60 dark:bg-rose-950/30">
          <div className="mb-4 flex size-12 items-center justify-center rounded-full bg-rose-100 dark:bg-rose-900/60">
            <AlertTriangle
              aria-hidden="true"
              className="size-5 text-rose-500 dark:text-rose-400"
            />
          </div>
          <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-50">
            {t("dealers.form.loadError")}
          </h3>
          <p className="mt-1 max-w-sm text-sm text-slate-500 dark:text-slate-400">
            {t("dealers.form.loadErrorDescription")}
          </p>
          <Link
            href="/dealers"
            className="mt-6 inline-flex items-center rounded-lg bg-slate-900 px-3.5 py-2 text-sm font-medium text-white shadow-sm transition-colors hover:bg-slate-700 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-200"
          >
            {t("dealers.form.backToList")}
          </Link>
        </div>
      )}

      {state.status === "ready" && (
        <div className="mx-auto w-full max-w-3xl">
          <DealerForm mode="edit" dealer={state.dealer} />
        </div>
      )}
    </PageContainer>
  );
}
