...
import { getManagerAnalytics } from "@/lib/dashboard/analytics";
...
export async function getManagerAnalyticsAction(): Promise<
...
> {
  ...
  try {
    const payload = await getManagerAnalytics(user.id);
    return ok(payload);
  } catch (error) {
