...
import { getAccountsTerritoryMap } from "@/lib/dashboard/maps";
...
export async function getAccountsTerritoryMapAction(
  ...
  try {
    const payload = await getAccountsTerritoryMap(user.id, filters);
    return ok(payload);
  } catch (error) {
