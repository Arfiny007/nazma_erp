...
import { retryNotification as retryNotificationRecord } from "@/lib/notifications";
...
export async function retryNotification(
  ...
  try {
    const actor = await requirePermission("notifications:retry");
    if (!notificationId?.trim()) {
      return fail("VALIDATION_ERROR", "notifications.error.idRequired");
    }

    const data = await retryNotificationRecord(notificationId.trim(), actor.id);
