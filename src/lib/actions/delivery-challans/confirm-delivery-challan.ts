"use server";

import { DeliveryChallanStatus } from "@prisma/client";
import { revalidatePath } from "next/cache";

import {
  assertCanConfirmChallan,
  assertChallanHasItems,
  assertNotOverDelivery,
  DeliveryWorkflowError,
} from "@/lib/delivery/workflow";
import { prisma } from "@/lib/prisma";
import { requirePermission } from "@/lib/rbac/guards";
import { confirmDeliveryChallanSchema } from "@/lib/validators/delivery-challan.schema";
import type { ActionResult, DeliveryChallanDetailDTO } from "@/types/delivery-challan";

import {
  ChallanActionError,
  fail,
  fromPrismaError,
  fromZodError,
  loadChallanDetailDTO,
  loadOrderLineSnapshots,
  mapWorkflowError,
  ok,
  recordChallanAudit,
  syncOrderStatusAfterChallanConfirm,
} from "./helpers";

/**
 * Confirms (dispatches) a Draft delivery challan.
 *
 * Re-validates over-delivery inside the transaction, sets `dispatchedAt`, and
 * synchronizes the parent order status (Approved → Partially_Delivered →
 * Delivered).
 *
 * NON-FINANCIAL: no invoice, ledger, balance, due, or collection side effects.
 */
export async function confirmDeliveryChallan(
  input: unknown,
): Promise<ActionResult<DeliveryChallanDetailDTO>> {
  let user;
  try {
    user = await requirePermission("orders:edit");
  } catch {
    return fail<DeliveryChallanDetailDTO>("FORBIDDEN", "rbac.noAccess");
  }

  const parsed = confirmDeliveryChallanSchema.safeParse(input);
  if (!parsed.success) {
    return fromZodError(parsed.error);
  }

  const { id } = parsed.data;

  try {
    await prisma.$transaction(async (tx) => {
      const existing = await tx.deliveryChallan.findUnique({
        where: { id },
        select: {
          id: true,
          orderId: true,
          status: true,
          _count: { select: { items: true } },
          items: { select: { orderItemId: true, quantity: true } },
        },
      });
      if (!existing) {
        throw new ChallanActionError(
          "CHALLAN_NOT_FOUND",
          "challan.error.notFound",
        );
      }

      assertCanConfirmChallan(existing.status);
      assertChallanHasItems(existing._count.items);

      const snapshots = await loadOrderLineSnapshots(tx, existing.orderId, id);
      assertNotOverDelivery(
        snapshots,
        existing.items.map((item) => ({
          orderItemId: item.orderItemId,
          quantity: item.quantity,
        })),
      );

      const dispatchedAt = new Date();

      await tx.deliveryChallan.update({
        where: { id },
        data: {
          status: DeliveryChallanStatus.Confirmed,
          dispatchedAt,
          confirmedById: user.id,
        },
      });

      await syncOrderStatusAfterChallanConfirm(tx, existing.orderId);

      await recordChallanAudit(tx, {
        userId: user.id,
        challanId: id,
        action: "DELIVERY_CHALLAN_CONFIRMED",
        oldValue: { status: existing.status },
        newValue: {
          status: DeliveryChallanStatus.Confirmed,
          dispatchedAt: dispatchedAt.toISOString(),
        },
      });
    });

    const detail = await loadChallanDetailDTO(id);
    if (!detail) {
      return fail<DeliveryChallanDetailDTO>(
        "CHALLAN_NOT_FOUND",
        "challan.error.notFound",
      );
    }

    revalidatePath("/orders");
    revalidatePath(`/orders/${detail.orderId}`);

    return ok(detail);
  } catch (error) {
    if (error instanceof ChallanActionError) {
      return fail<DeliveryChallanDetailDTO>(error.code, error.messageKey);
    }
    if (error instanceof DeliveryWorkflowError) {
      return mapWorkflowError(error);
    }
    return fromPrismaError(error);
  }
}
