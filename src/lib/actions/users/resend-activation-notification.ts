...
import { resendActivationNotification } from "@/lib/notifications/auth-notifications";
...
export async function resendActivationEmail(
  ...
  try {
    ...
    const data = await resendActivationNotification(actor, userId);
    ...
    if (error instanceof AuthNotificationUserNotEligibleError) {
      return fail("VALIDATION_ERROR", "userManagement.error.resendActivationNotEligible");
    }
